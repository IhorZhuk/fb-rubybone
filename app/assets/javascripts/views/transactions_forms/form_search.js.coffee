FamilyBudget.Views.TransactionsFormSearch = Marionette.View.extend
  ...
  setQuery: ->
    ...
    @query.title = @ui.title.val()
    @query.amount = @ui.amount.val()
    @query.kind = @ui.kind.val()
    @query.category_id = @ui.categories.val()
    @query.note = @ui.note.val()
    @query.date_from = @ui.dateFrom.val()
    @query.date_to = @ui.dateTo.val()
